...
class TestingLinks(unittest.TestCase):
    def test_create_new_link(self):
        ...
        self.assertEquals(links[0].source_x, 0)
        self.assertEquals(links[0].source_y, 0)
        self.assertEquals(links[0].source_link_id, 0)
        self.assertEquals(links[0].destination_x, 0)
        self.assertEquals(links[0].destination_y, 1)
        self.assertEquals(links[0].multicast_default_from, s)
        self.assertEquals(links[0].multicast_default_to, s)
